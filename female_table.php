...
<span>Child Information Table (Female)</span>
...
<?php } else {
	...
	$result_per_page = 6;
	$sex = "Female";
	$show_child_information = $conn->prepare("SELECT *FROM child_information WHERE Sex = '$sex'" );
	...
	$this_page_first_result = ($page-1) * $result_per_page;
	$sex = "Female";
	$show_child_information = $conn->query("SELECT *FROM child_information WHERE Sex = '$sex' ORDER BY Height DESC LIMIT ".$this_page_first_result.','.$result_per_page);
...
// Else if current_page is 
	...
	$current_page--;
	  ...
	  echo '<a href="female_table.php?page='.($current_page).'">'.'Previous'.'</a>';
 ...
 for($i = max(1, $page - 3); $i<=min($page + 1, $number_of_pages); $i++) {
  ...
  echo '<li class='.$activeClass.'>'.'<a  href="female_table.php? page='.$i.'">'.$i.'</a>'.'</li>';
...
if($current_page >= $number_of_pages) {
  ...
  echo '<a href="female_table.php? page='.($current_page).'">'.'Next'.'</a>';
