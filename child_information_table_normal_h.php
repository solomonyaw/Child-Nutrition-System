...
<html>
	...
	<!--TITLE-->
	<title>Table (Normal Height)</title>
	<!--TITLE ENDS HERE-->
...
<span>Child Information of Centro Oriental (Normal Height)</span>
...
<?php } else {
	...
	$result_per_page = 6;
	$normal = "Normal";
	$show_child_information = $conn->prepare("SELECT *FROM child_information WHERE Height_Status LIKE '%".$normal."%'" );
	...
	$this_page_first_result = ($page-1) * $result_per_page;
	$normal = "Normal";
	$show_child_information = $conn->query("SELECT *FROM child_information WHERE Height_Status LIKE '%".$normal."%'  ORDER BY Name ASC LIMIT ".$this_page_first_result.','.$result_per_page);
...
// Else if current_page is 
	...
	$current_page--;
	  ...
	  echo '<a href="child_information_table_normal_h.php?page='.($current_page).'">'.'Previous'.'</a>';
 ...
 for($i = max(1, $page - 3); $i<=min($page + 1, $number_of_pages); $i++) {
  ...
  echo '<li class='.$activeClass.'>'.'<a  href="child_information_table_normal_h.php? page='.$i.'">'.$i.'</a>'.'</li>';
...
if($current_page >= $number_of_pages) {
  ...
  echo '<a href="child_information_table_normal_h.php? page='.($current_page).'">'.'Next'.'</a>';
