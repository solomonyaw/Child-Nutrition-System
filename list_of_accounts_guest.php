...
if(isset($_GET['edit_guest_success'])){
	
	
							echo '<script>';
							echo 'alert("Guest Account Succesfully Updated.")';
							echo '</script>';

}

if(isset($_GET['delete_guest_success'])){
	
	
							echo '<script>';
							echo 'alert("Guest Account Succesfully Deleted.")';
							echo '</script>';
...
<html>
	...
	<!--TITLE-->
	<title>List of Guest Accounts</title>
	<!--TITLE ENDS HERE-->
...
<script>
	...
	//EDIT
			function edit_user_account_guest(id, user){
				if (confirm("Are you sure you want to edit?  '"+user + " '"))
				{
					
					window.location.href = 'edit_user_account_guest.php?edit_user_account_guest=' +id;
				}
			}
			...
			//UPDATE	
			function delete_user_account_guest(id, user){
				if (confirm("Are you sure you want to delete  '" + user + "'"))
				{
				window.location.href = 'delete_user_account_guest.php?delete_user_account_guest=' + id;
					}	
				}
...
<a href="list_of_accounts.php">User</a>
<a class="active" href="list_of_accounts_guest.php">Guest</a>
...
<table class="table_guest guest_responsive" cellpadding="0" cellspacing="0" border="0">
		...
		<tbody  cellpadding="0" cellspacing="0" border="0">
			...
			$show_accounts = $conn-> prepare("SELECT *FROM user_account_guest");
			...
			$show_guest_accounts = $conn->query("SELECT *FROM user_account_guest WHERE FirstName LIKE '%".$search_final_data."%' OR MiddleName LIKE '%".$search_final_data."%' OR LastName LIKE '%".$search_final_data."%' OR Username LIKE '%".$search_final_data."%' OR Occupation LIKE '%".$search_final_data."%' OR Reason LIKE '%".$search_final_data."%' ORDER BY FirstName ASC LIMIT ".$this_page_first_result.','.$result_per_page);
			
			$count_result = $show_guest_accounts->rowCount();
			...
			while($row=$show_guest_accounts->fetch()){
			...
			<td>
			<!-- EDIT BUTTON -->
			<a href="javascript:edit_user_account_guest('<?php echo $row['IDNumber'];?>' , '<?php echo $row['UserName'];?>')"> 
			...
			<td>
			<!-- DELETE BUTTON -->
			<a href="javascript:delete_user_account_guest('<?php echo $row['IDNumber'];?>' , '<?php echo $row['UserName'];?>')"> 			
			...
			$show_guest_accounts = $conn->query("SELECT *FROM user_account_guest ORDER BY FirstName ASC LIMIT ".$this_page_first_result.','.$result_per_page);
			
			$count_guest_account = $show_guest_accounts->rowCount();
			
			if(empty($count_guest_account)){
				
				 echo 
					 '<tr >
						<td><p> No Guest Account Created </p></td>
					 </tr>';
			...
			<?php
			echo '<span>'.$row['FirstName'].' '.$row['MiddleName'].' '.$row['LastName'].'</span>'.'</td>';
			echo '<td>'.$row['Occupation'].'</td>';
			echo '<td>'.$row['Reason'].'</td>';
...
// Else if curre_page is 
	...
	$current_page--;
	  ...
	  echo '<a href="list_of_accounts_guest.php?page='.($current_page).'">'.'Previous'.'</a>';
 ...
 for($i = max(1, $page - 3); $i<=min($page + 1, $number_of_pages); $i++) {
  ...
  echo '<li class='.$activeClass.'>'.'<a  href="list_of_accounts_guest.php? page='.$i.'">'.$i.'</a>'.'</li>';
...
if($current_page >= $number_of_pages) {
  ...
  echo '<a href="list_of_accounts_guest.php? page='.($current_page).'">'.'Next'.'</a>';
