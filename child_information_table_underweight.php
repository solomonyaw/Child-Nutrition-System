...
<span>Child Information of Centro Oriental (Underweight)</span>
...
<?php } else {
	...
	$show_child_information = $conn->prepare("SELECT *FROM child_information WHERE Weight_Status LIKE '%".$underweight."%'" );
	...
	$this_page_first_result = ($page-1) * $result_per_page;
	$underweight = "Underweight";
	$show_child_information = $conn->query("SELECT *FROM child_information WHERE Weight_Status LIKE '%".$underweight."%'  ORDER BY Name ASC LIMIT ".$this_page_first_result.','.$result_per_page);
...
// Else if current_page is 
	...
	$current_page--;
	  ...
	  echo '<a href="child_information_table_underweight.php?page='.($current_page).'">'.'Previous'.'</a>';
 ...
 for($i = max(1, $page - 3); $i<=min($page + 1, $number_of_pages); $i++) {
  ...
  echo '<li class='.$activeClass.'>'.'<a  href="child_information_table_underweight.php? page='.$i.'">'.$i.'</a>'.'</li>';
...
if($current_page >= $number_of_pages) {
  ...
  echo '<a href="child_information_table_underweight.php? page='.($current_page).'">'.'Next'.'</a>';
